...
Two pages further on it also says the read opcode is `0x0B` and not `0x03` — `0x03` is the read opcode of flash memory chips and not a command on this controller at all.
...
The three deviations did not turn up where you expect deviations, but in exactly the three functions you wanted to know about.
